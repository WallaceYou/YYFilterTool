...
typedef NS_ENUM(NSUInteger, YYFilterToolType) {
    YYFilterToolTypeSingleLevel = 1,
    YYFilterToolTypeDoubleLevel = 2,
};

@interface YYFilterTool : NSObject

/** 筛选层级，一层还是两层，目前支持两层，以后有需求可以拓展到三层 */
@property (nonatomic, assign) YYFilterToolType levelType;
